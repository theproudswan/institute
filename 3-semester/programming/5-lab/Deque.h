...
// Автор       : Лебедев Иван
// Группа      : М3О-211Б-20
// Содержание  : Прототип и определение класса Deque
//============================================================================
...
public:					// Публичная секция
	...
	void Print() const;                 // Метод для печати дека
    size_t GetSize() const;             // Константный метод получения размера дека
...
template <typename T>    // Шаблон метода
void Deque<T>::Print() const { // Метод для печати дека
    ...
    for (size_t i = 0; i < data.GetSize(); ++i) {   // Цикл для перебора всего дека
        cout << "deque[" << i << "] = " << data[i] -> value << endl; // Вывести соответствующий элемент
    } // Конец цикла
} // Конец метода Print


template <typename T>    // Шаблон метода
size_t Deque<T>::GetSize() const { // Константный метод получения размера дека
    return data.GetSize();  // Вернуть размер списка
} // Конец константного метода GetSize