...
template <typename T> // Шаблон метода
  void LinkedList<T>::PushFront(const T& value){ // Метод добавляет в конец элемент со значем value
    ...
    n_ptr->prev = nullptr;         // У вспомогательного узла нет предыдущего
    ...
    head->prev = tail;             // Узел перед головой - хвост
...
template <typename T> // Шаблон метода
  void LinkedList<T>::PushBack(const T& value){ // Метод класса добавляет узел со значением value в конец списка
	  ...
	  n_ptr->prev = tail;             // Предыдущий узел у вспомогательного узла - хвост
    ...
    head->prev = tail;              // Узел перед головой списка - новый хвост
	  size++;                         // Увеличить счётчик элементов
...
template <typename T> // Шаблон метода класса
  void LinkedList<T>::InsertAfter(Node* node, const T& value){ // Метод класса вставляет узел со значением value перед узлом node
      if(node == nullptr){            // Если node пуст
        ...
        tmp->prev = node;             // Предыдущим элементом для вспомогательного послужит сам узел node
  ...
  void LinkedList<T>::RemoveAfter(Node* node){  // Метод класса удаляет узел после узла node
    ...
    if (node == tail){   // Если node пуст
      ...
      if((node->next) != nullptr){  // Если элемент после node не пуст
        ...
        node->prev = tmp->prev;     // Узел перед node равен узлу перед вспомогательным
...
template <typename T> // Шаблон метода класса
  void LinkedList<T>::PopFront(){ // Удаление головы списка
    ...
    if(head){                     // Если есть первый элемент
      ...
      head->prev = tail;          // Узел перед новой головой - старый хвост
...
template <typename T> // Шаблон метода класса
  void LinkedList<T>::PopBack(){ // Удаление хвоста списка
	  ...
	  if(tail){                    // Если есть последний элемент
	      ...
	      Node* n_ptr = tail->prev;// Завести вспомогательный узел, куда записать узел перед хвостом
...
template <typename T> // Шаблон метода класса
  size_t LinkedList<T>::UpperBound(const T& to_find) const {  // Константный метод поиска последнего вхождения элемента to_find
      if (IsEmpty()){                   // Если список пуст
  	  ...
  	  Node* tmp = tail;             // Вспомогательный узел установить на конец списка
  	  size_t index = GetSize() - 1; // Индекс установить на последний элемент
  	  while(tmp != head && tmp){        // Пока вспомогательный узел не равен голове списка и список не пустой
  	  		  if(tmp->value == to_find){  // Если значение вспомогательного узла равно искомому
  	  			  return index;         // Вернуть индкес этого элемента
  	  		  }                       // Конец Если
  	  		  tmp = tmp->prev;        // Перейти к предыдущему элементу
  	  		  index--;                // Уменьшить индекс
  	  }                             // Конец Пока
      if(tmp->value == to_find){    // Если голова списка равна искомому элементу
	  		return index;               // Вернуть индекс головы списка
	  	}                             // Конец Если
  	  return GetSize();             // Вернуть размер списка
...
template <typename T> // Шаблон перегрузки оператора
  typename LinkedList<T>::Node* LinkedList<T>::operator[] (const size_t index) const { // Перегрузка оператора [] для константного доступа к элементу списка по егу индексу
	  Node* tmp;    // Вспомогательный узел
	  size_t i;     // Индекс текущего элемента
    if (index <= GetSize() / 2){  // Если элемент с нужным инедксом в первой половине
      tmp = head;       // Установить вспомогательный узел на голову списка
      i = 0;            // Начинаем перебирать с начального элемента
    ...
    } else {            // В противом случае (элемент с нужным индексом во второй половине)
      i = GetSize() - 1;// Начинаем перебирать с конца
      tmp = tail;       // Установить вспомогательный элемент на хвост списка
      while(i > index % GetSize()){ // Пока индекс текущего элемента больше искомого (остаток от деления нужен, т.к список кольцевой)
  	  		  tmp = tmp->prev;        // Перейти к предыдущему элементу
  	  		  i--;                    // Уменьшить индекс
  	  }                             // Конец Пока
    }                   // Конец Если
